...
class ConnectToAvroConverterTest {
    ...
    inner class SchemaConversion {
        ...
        fun `converts Connect Date to Avro date`() {
            ...
            avro.getField("d").schema().logicalType.name shouldBe "date"
        }
        ...
        fun `converts Connect Timestamp to timestamp-micros`() {
            ...
            avro.getField("ts").schema().logicalType.name shouldBe "timestamp-micros"
        }

        @Test
        fun `converts Debezium MicroTimestamp to timestamp-micros`() {
            val tsSchema = SchemaBuilder.int64()
                .name("io.debezium.time.MicroTimestamp").build()
            val connectSchema = SchemaBuilder.struct()
                .field("ts", tsSchema).build()
        ...
        fun `converts Debezium Date to Avro date`() {
            val dateSchema = SchemaBuilder.int32()
                .name("io.debezium.time.Date").build()
            val connectSchema = SchemaBuilder.struct()
                .field("d", dateSchema).build()
        ...
        fun `converts Debezium NanoTimestamp to timestamp-micros`() {
            val tsSchema = SchemaBuilder.int64()
                .name("io.debezium.time.NanoTimestamp").build()
            val connectSchema = SchemaBuilder.struct()
                .field("ts", tsSchema).build()
        ...
        fun `converts Debezium ZonedTimestamp to string`() {
            val tsSchema = SchemaBuilder.string()
        ...
        fun `converts Debezium time types to long`() {
            val microTime = SchemaBuilder.int64()
                .name("io.debezium.time.MicroTime").build()
            val time = SchemaBuilder.int32()
                .name("io.debezium.time.Time").build()
            val nanoTime = SchemaBuilder.int64()
                .name("io.debezium.time.NanoTime").build()
            val connectSchema = SchemaBuilder.struct()
                .field("t1", microTime)
                .field("t2", time)
                .field("t3", nanoTime)
        ...
        fun `converts bytes type`() {
            val connectSchema = SchemaBuilder.struct()
                .field("data", Schema.BYTES_SCHEMA).build()
    ...
    inner class DataConversion {
        ...
        fun `converts Debezium VariableScaleDecimal to string`() {
            ...
            val vsd = Struct(vsdSchema)
                .put("scale", 3)
                .put("value", java.math.BigInteger.valueOf(12345L).toByteArray())
        ...
        fun `skips fields not in Avro schema`() {
            val fullSchema = SchemaBuilder.struct()
                .field("id", Schema.INT32_SCHEMA)
                .field("__deleted", Schema.OPTIONAL_STRING_SCHEMA)
                .build()
            val filteredConnect = SchemaBuilder.struct()
