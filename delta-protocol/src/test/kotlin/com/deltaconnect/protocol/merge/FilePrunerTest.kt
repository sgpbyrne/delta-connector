...
class FilePrunerTest {
    ...
    private fun intStats(column: String, min: Int, max: Int, numRecords: Int = 10): String =
        """{"numRecords":$numRecords,""" +
            """"minValues":{"$column":$min},""" +
            """"maxValues":{"$column":$max},""" +
            """"nullCount":{"$column":0}}"""

    private fun longStats(column: String, min: Long, max: Long): String =
        """{"numRecords":10,""" +
            """"minValues":{"$column":$min},""" +
            """"maxValues":{"$column":$max},""" +
            """"nullCount":{"$column":0}}"""

    private fun stringStats(column: String, min: String, max: String): String =
        """{"numRecords":10,""" +
            """"minValues":{"$column":"$min"},""" +
            """"maxValues":{"$column":"$max"},""" +
            """"nullCount":{"$column":0}}"""
    ...
    inner class MultipleFiles {
        ...
        fun `correctly partitions matching and pruned files`() {
            ...
            result.matchingFiles.map { it.path }
